...
void PrintHelpGuide() {
	...
	cout << "1) \"BATTLESHIP\" SHIPS" << endl;
	cout << "2) \"BATTLESHIP\" GAME PLAY" << endl;
	cout << "3) \"BATTLESHIP\" HIGH SCORES" << endl;
	cout << "4) \"BATTLESHIP\" CREDITS" << endl;
	cout << "5) \"BATTLESHIP\" BACK" << endl;

	cout<<"Please enter a number to read the instructions:";
	int NumberChoice;
	...
	case 1:
		...
		cout << "\t\tSHIP NAMES\t\tLENGTHS\t\tCHARACTERS" << endl;
	...
	case 2:
		...
		cout << "\t\t\ta) From the main menu, press '1' to begin a new game."
				<< endl;
		cout
				<< "\t\t\tb) Enter your first name. Enter the start and end coordinates of each ship beginning with the longest and ending with the shortest."
				<< endl;
		cout
				<< "\t\t\t\ti) Valid coordinate entries: A1 E1, A1E1, a1 e1, a1e1, A 1 E 1, a 1 e 1"
				<< endl;
		cout
				<< "\t\t\t\tii) The user's updated grid with ship locations will be displayed after each ship placement."
				<< endl;
		cout
				<< "\t\t\t\tiii) Any invalid ship placements will prompt the user to re-enter ship coordinates."
				<< endl;
		cout
				<< "\t\t\tc) Game play will begin. User will be prompted to enter one coordinate to fire on the opponent's grid."
				<< endl;
		cout
				<< "\t\t\t\ti) The opponent's grid will be updated with 'X' for a hit and 'M' for a miss."
				<< endl;
		cout
				<< "\t\t\t\tii) The opponent will select a grid location after the user has selected a location."
				<< endl;
		cout
				<< "\t\t\t\tiii) At any time during the game the user may enter '?' to view the Help Guide, 'S' to save and exit the game, or 'Q' to quit the current game without saving."
				<< endl;
		cout
				<< "\t\t\t\tiv) Game play will continue until either the user or opponent wins or until the user chooses to save and quit or quit without saving."
				<< endl;
		cout
				<< "\t\t\t\tv) *The user may only save one game at a time. If a game has already been saved, the user may save their current game and the old saved game will be overwritten."
				<< endl;
		cout << "\t\t2) Load a Saved Game" << endl;
		cout << "\t\t\ta) From the main menu, press '2' to load a saved game."
				<< endl;
		cout
				<< "\t\t\tb) Only one game may be saved at a time. *If more than one game is saved, the older one will be overwritten by the newer game. The newest game will be loaded and game play will continue at the last saved point. Both the user's and opponent's grid will be displayed with all of the saved information and coordinates that have previously been played."
				<< endl;
		cout << endl;
	...
	case 3:
		...
		cout
				<< "\t\t\tc) When user sinks a ship, plus a ship bonus of 10 times the length of the ship."
				<< endl;
		cout
				<< "\t\t\td) If the user wins, their final score is their current scores times 100\\% plus their hit percentage (ratio of hits to misses)."
				<< endl;
		cout << "\t\t\te) If the user loses, then their final score is 0."
				<< endl;
		cout << "\t\t2) View High Scores" << endl;
		cout << "\t\t\ta) From the main menu, press '3' to view high scores."
				<< endl;
		cout
				<< "\t\t\tb) The top 10 highest scores will be displayed and ranked along with the user's name."
				<< endl;
		cout << endl;
	...
	case 4:
		...
		cout
				<< "\t\t\ta) The Triad Development Group (TDG) created the terminal based computer generated \"BATTLESHIP\" game for the HASBRO company."
				<< endl;
		cout
				<< "\t\t\t\ti) Credit for the creation of \"BATTLESHIP\" goes to Michele Burns, Project Manager; Joshua Gilstrap, Client/Developer Liason; & Greyson Whitaker, Documents Manager."
				<< endl;
		cout
				<< "\t\t2) \"BATTLESHIP\" is a game of the HASBRO company, all rights reserved. Electronic terminal-based version of \"BATTLESHIP\" was created by the Triad Development Group, Inc. for the HASBRO company, in cooperation with the Senior Manager, Dr. Scott Russell, of High Point University's Department of Mathematics and Computer Science, CSC 3710: Software Engineering & Design."
				<< endl;
